...
type StatServeAPI [c2t_idcmd.CommandID_Count]StatRow

func New() *StatServeAPI {
	ps := new(StatServeAPI)
	for i := 0; i < c2t_idcmd.CommandID_Count; i++ {
		ps[i].Name = c2t_idcmd.CommandID(i).String()
	}
	return ps
}
func (ps *StatServeAPI) AfterRecvReqHeader(header c2t_packet.Header) (*StatObj, error) {
	if int(header.Cmd) >= c2t_idcmd.CommandID_Count {
		return nil, fmt.Errorf("CommandID out of range %v %v", header, c2t_idcmd.CommandID_Count)
	}
...
func (sm *StatObj) AfterSendRsp(hd c2t_packet.Header) {
...
func (sr *StatRow) open(hd c2t_packet.Header) *StatObj {
	sr.mutex.Lock()
	defer sr.mutex.Unlock()
	sr.RecvCount++
	rxbyte := int(hd.BodyLen()) + c2t_packet.HeaderLen
	sr.RecvBytes += rxbyte
...
func (sr *StatRow) afterSend(diffDur time.Duration, hd c2t_packet.Header) {
	sr.mutex.Lock()
	defer sr.mutex.Unlock()
	sr.SendCount++
	txbyte := int(hd.BodyLen()) + c2t_packet.HeaderLen
	sr.SendBytes += txbyte
