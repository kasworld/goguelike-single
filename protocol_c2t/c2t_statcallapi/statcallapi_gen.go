...
type StatCallAPI [c2t_idcmd.CommandID_Count]StatRow

func New() *StatCallAPI {
	cps := new(StatCallAPI)
	for i := 0; i < c2t_idcmd.CommandID_Count; i++ {
		cps[i].Name = c2t_idcmd.CommandID(i).String()
	}
	return cps
}
func (cps *StatCallAPI) BeforeSendReq(header c2t_packet.Header) (*statObj, error) {
	if int(header.Cmd) >= c2t_idcmd.CommandID_Count {
		return nil, fmt.Errorf("CommandID out of range %v %v",
			header, c2t_idcmd.CommandID_Count)
	}
	return cps[header.Cmd].open(), nil
}
func (cps *StatCallAPI) AfterSendReq(header c2t_packet.Header) error {
	if int(header.Cmd) >= c2t_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, c2t_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + c2t_packet.HeaderLen
	cps[header.Cmd].addTx(n)
	return nil
}
func (cps *StatCallAPI) AfterRecvRsp(header c2t_packet.Header) error {
	if int(header.Cmd) >= c2t_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, c2t_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + c2t_packet.HeaderLen
