...
type ErrorCodeStat [c2t_error.ErrorCode_Count]int
...
func (es ErrorCodeStat) String() string {
	...
	for i, v := range es {
		fmt.Fprintf(&buf,
			"%v:%v ",
			c2t_error.ErrorCode(i), v)
	}
	buf.WriteString("]")
	return buf.String()
}
func (es *ErrorCodeStat) Inc(e c2t_error.ErrorCode) {
	es[e] += 1
}
func (es *ErrorCodeStat) Add(e c2t_error.ErrorCode, v int) {
	es[e] += v
}
func (es *ErrorCodeStat) SetIfGt(e c2t_error.ErrorCode, v int) {
	if es[e] < v {
		es[e] = v
	}
}
func (es ErrorCodeStat) Get(e c2t_error.ErrorCode) int {
	return es[e]
}

// Iter return true if iter stop, return false if iter all
// fn return true to stop iter
func (es ErrorCodeStat) Iter(fn func(i c2t_error.ErrorCode, v int) bool) bool {
	for i, v := range es {
		if fn(c2t_error.ErrorCode(i), v) {
			return true
		}
	}
	return false
...
func Index(i int) string {
	return c2t_error.ErrorCode(i).String()
}
