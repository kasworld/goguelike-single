...
func (app *WasmClient) makeBaseInfoHTML() string {
	...
	fmt.Fprintf(&buf, "%s %.2f/%.2f", wrapspan.ColorTextf("Lime", "AP"), pao.AP, leveldata.MaxAP(int(lv)))
	buf.WriteString("<br/>")
...
func (app *WasmClient) MakeNotiMessage() string {
	...
	winH := win.Get("innerHeight").Float()
	refsize := winH / 50
	// jslog.Infof("len app.NotiMessage %v", len(app.NotiMessage))
	return app.NotiMessage.ToHtml(refsize)
}
