...
type TableClickEvent struct {
	appwindow.MouseEvent         // Embedded appwindow mouse event
...
func NewTable(width, height float32, cols []TableColumn) (*Table, error) {
	...
	for ci := 0; ci < len(cols); ci++ {
		...
		if c.sort != TableSortNone {
			...
			c.Add(c.ricon)
			c.ricon.Subscribe(eventtype.OnMouseDown, func(evname eventtype.EventType, ev interface{}) {
				t.onRicon(evname, c)
			})
		}
	...
	// Subscribe to events
	t.Panel.Subscribe(eventtype.OnCursor, t.onCursorPos)
	t.Panel.Subscribe(eventtype.OnScroll, t.onScroll)
	t.Panel.Subscribe(eventtype.OnMouseUp, t.onMouse)
	t.Panel.Subscribe(eventtype.OnMouseDown, t.onMouse)
	t.Panel.Subscribe(eventtype.OnKeyDown, t.onKey)
	t.Panel.Subscribe(eventtype.OnKeyRepeat, t.onKey)
	t.Panel.Subscribe(eventtype.OnResize, t.onResize)
	t.recalc()
	return t, nil
}
...
func (t *Table) InsertRow(row int, values map[string]interface{}) {
	...
	t.recalc()
	t.Dispatch(eventtype.OnTableRowCount, nil)
}
...
func (t *Table) scrollDown(n int) {
	...
	if t.rowCursor < t.firstRow {
		...
		t.Dispatch(eventtype.OnChange, nil)
	}
	t.recalc()
	return
}
...
func (t *Table) onCursorPos(evname eventtype.EventType, ev interface{}) {

	// Convert mouse appwindow coordinates to table content coordinates
	...
	// resets the the appwindow cursor.
...
func (t *Table) onMouse(evname eventtype.EventType, ev interface{}) {
	...
	case eventtype.OnMouseDown:
		...
		if t.resizeCol >= 0 && e.Button == appwindow.MouseButtonLeft {
			t.resizing = true
		...
		if tce.Row >= 0 && e.Button == appwindow.MouseButtonLeft {
			t.rowCursor = tce.Row
			if t.selType == TableSelMultiRow && e.Mods == appwindow.ModControl {
		...
		t.Dispatch(eventtype.OnTableClick, tce)
...
func (t *Table) onKey(evname eventtype.EventType, ev interface{}) {

	kev := ev.(*appwindow.KeyEvent)
	if kev.Key == appwindow.KeyUp && kev.Mods == 0 {
		t.selPrev()
	} else if kev.Key == appwindow.KeyDown && kev.Mods == 0 {
		t.selNext()
	} else if kev.Key == appwindow.KeyPageUp && kev.Mods == 0 {
		t.prevPage()
	} else if kev.Key == appwindow.KeyPageDown && kev.Mods == 0 {
		t.nextPage()
	} else if kev.Key == appwindow.KeyPageUp && kev.Mods == appwindow.ModControl {
		t.firstPage()
	} else if kev.Key == appwindow.KeyPageDown && kev.Mods == appwindow.ModControl {
		t.lastPage()
	} else if kev.Key == appwindow.KeyEnter && kev.Mods == appwindow.ModControl {
...
func (t *Table) onResize(evname eventtype.EventType, ev interface{}) {
...
func (t *Table) onScroll(evname eventtype.EventType, ev interface{}) {
...
func (t *Table) onRicon(evname eventtype.EventType, c *tableColHeader) {
...
func (t *Table) setVScrollBar(state bool) {
	...
	if state {
		...
		if t.vscroll == nil {
			...
			t.vscroll.Subscribe(eventtype.OnChange, t.onVScrollBar)
...
func (t *Table) onVScrollBar(evname eventtype.EventType, ev interface{}) {
